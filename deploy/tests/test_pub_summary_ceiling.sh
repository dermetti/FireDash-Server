#!/usr/bin/env bash
# Regression test: the repo-managed PUBLICATION_BUILD_SUMMARY_MAX_ITEMS ceiling
# converges to 50000 on render, is idempotent on rerun, and never overwrites an
...
# Existing env: old canonical ceiling + an operator-customized replacement value.
cat > "$ENV_FILE" <<'EOF'
...
grep -qx 'PUBLICATION_BUILD_SUMMARY_MAX_ITEMS=50000' "$ENV_FILE" \
    || fail "ceiling did not converge to 50000"
...
grep -qx 'PUBLICATION_BUILD_SUMMARY_MAX_ITEMS=50000' "$ENV_FILE" \
    || fail "clean install did not render 50000"

if [[ $failures -eq 0 ]]; then
    echo "ok: publication summary ceiling converges to 50000 idempotently"
else
    exit 1
fi
